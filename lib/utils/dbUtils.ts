...
export const insert = async (table: Table, row: any): Promise<string> => {
    ...
    try {
        ...
        const res = await client.db(dbName).collection(table).insertOne(row);
...
export const getCollection = async <T>(table: Table): Promise<T[]> => {
    ...
    try {
        await client.connect();
        results = await client.db(dbName).collection(table).find().toArray() as T[];
    } catch (e) {
        console.error(e);
    } finally {
        await client.close();
